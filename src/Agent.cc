...
namespace RVO
{
        ...
        { /* NOLINT(runtime/references) */
            ...
            for (std::size_t i = beginLine; i < lines.size(); ++i)
            {
                if (det(lines[i].direction, lines[i].point - result) > distance)
                {
                    ...
                    if (linearProgram2(
                        projLines, radius, Vector2(-lines[i].direction.y(), lines[i].direction.x()), true, result) < projLines.size())
                    {
    ...
    void Agent::computeNewVelocity(float timeStep)
    {
        ...
        for (std::size_t i = 0U; i < obstacleNeighbors_.size(); ++i)
        {
            ...
            const float t =
            obstacle1 == obstacle2 ? 0.5F : (velocity_ - leftCutoff) * cutoffVector / absSq(cutoffVector);
            ...
            (t < 0.0F || t > 1.0F || obstacle1 == obstacle2) ? std::numeric_limits<float>::infinity() : absSq(velocity_ - (leftCutoff + t * cutoffVector));
            const float distSqLeft =
            tLeft < 0.0F ? std::numeric_limits<float>::infinity() : absSq(velocity_ - (leftCutoff + tLeft * leftLegDirection));
            const float distSqRight =
            tRight < 0.0F ? std::numeric_limits<float>::infinity() : absSq(velocity_ - (rightCutoff + tRight * rightLegDirection));
            ...
            if (distSqCutoff <= distSqLeft && distSqCutoff <= distSqRight)
            {
                ...
                line.direction = -obstacle1->direction_;
                line.point =
                leftCutoff + radius_ * invTimeHorizonObst * Vector2(-line.direction.y(), line.direction.x());
                orcaLines_.push_back(line);
                continue;
            }
            ...
            line.point =
            rightCutoff + radius_ * invTimeHorizonObst * Vector2(-line.direction.y(), line.direction.x());
            orcaLines_.push_back(line);
        }
    ...
    void Agent::insertObstacleNeighbor(const Obstacle* obstacle, float rangeSq)
    {
        ...
        {
            distSq = absSq(position_ - (obstacle->point_ + r * (nextObstacle->point_ - obstacle->point_)));
        }
