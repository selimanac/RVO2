...
namespace RVO
{
    ...
    void KdTree::buildAgentTreeRecursive(std::size_t begin, std::size_t end, std::size_t node)
    {
        ...
        if (end - begin > RVO_MAX_LEAF_SIZE)
        {
            ...
            0.5F * (isVertical ? agentTree_[node].maxX + agentTree_[node].minX : agentTree_[node].maxY + agentTree_[node].minY);
            ...
            while (left < right)
            {
                ...
                while (right > left &&
                       (isVertical ? agents_[right - 1U]->position_.x() : agents_[right - 1U]->position_.y()) >= splitValue)
                {
                    --right;
                }
    ...
    bool KdTree::queryVisibility(const Vector2& vector1, const Vector2& vector2, float radius) const
    {
